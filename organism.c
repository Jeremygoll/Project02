...
#define NUM_RECENT_INPUT 3        // number of recent inputs to consider when checking recent tasks
...
#ifdef _OPENMP
    static inline int get_thread_num() { return omp_get_thread_num(); }
#else
...
spawned_org_array* spawned_org_array_init(spawned_org_array* arr) {
    arr->data = (spawned_organism*)malloc(16*sizeof(spawned_organism));
    if (!arr->data) { perror("error allocating spawned array"); return NULL; }
    arr->size = 0;
    arr->capacity = 16;
    return arr;
}

spawned_organism* spawned_org_array_get_next(spawned_org_array* arr) {
    // grow storage space if necessary
    ...
    // grab the next spawned organism
...
bool environment_load_from_path(environment* env, const char* path, unsigned int random_seed, int num_threads) {
    ...
    bool retval = environment_load_from_file(env, file, random_seed, num_threads);
    fclose(file);
    return retval;
}

bool environment_load_from_file(environment* env, FILE* file, unsigned int random_seed, int num_threads) {
    // make sure the pointers are NULL before allocating memory
    env->grid = NULL;
    env->tasks_completed = NULL;
    env->spawned = NULL;
    ...
    if (!environment_alloc_grid(env, random_seed)) { return false; }

    // allocate memory for the spawned organisms
    env->spawned = (spawned_org_array*)malloc(num_threads*sizeof(spawned_org_array));
    if (!env->spawned) { perror("error allocating spawned array"); environment_free(env); return false; }
    memset(env->spawned, 0, num_threads*sizeof(spawned_org_array));
    for (int i = 0; i < num_threads; i++) {
        if (!spawned_org_array_init(&env->spawned[i])) {
            environment_free(env);
            return false;
        }
    }

    // allocate memory for the tasks completed
    env->tasks_completed = (task_counts*)malloc(num_threads*sizeof(task_counts));
    if (!env->tasks_completed) { perror("error allocating tasks_completed array"); environment_free(env); return false; }
    memset(env->tasks_completed, 0, num_threads*sizeof(task_counts));
...
void environment_free(environment* env) {
    free(env->grid);
    if (env->spawned) {
        for (int i = 0; i < env->num_threads; i++) { free(env->spawned[i].data); }
        free(env->spawned);
    }
    free(env->tasks_completed);
}
...
bool environment_alloc_grid(environment* env, unsigned int random_seed) {
    ...
    organism* grid = env->grid = (organism*)malloc(nbytes);
    if (!grid) { perror("error allocating grid"); return false; }
    memset(grid, 0, nbytes);
    for (uint32_t i = 0; i < size; i++) {
        for (uint32_t j = 0; j < size; j++) {
            uint32_t loc = i*size+j;
            grid[loc].env = env;
            grid[loc].location_i = i;
            grid[loc].location_j = j;
            grid[loc].random_seed = random_seed + loc;
        }
    }
    return true;
}
...
void environment_clear_tasks(environment* env) {
    memset(env->tasks_completed, 0, env->num_threads*sizeof(task_counts));
}

void environment_combine_tasks(environment* env) {
    for (int i = 1; i < env->num_threads; i++) {
        for (int j = 0; j < NUM_TASKS; j++) {
            env->tasks_completed[0].counts[j] += env->tasks_completed[i].counts[j];
        }
        memset(env->tasks_completed[i].counts, 0, sizeof(task_counts));
    }
}
...
void organism_clear(organism* org) {
    if (org->size) {
        ...
        unsigned int random_seed = org->random_seed;
        memset(org, 0, sizeof(organism));
        org->env = env;
        org->location_i = i;
        org->location_j = j;
        org->random_seed = random_seed;
    }
}
 ...
 * constants TASK_* such as TASK_OR and TASK_EQUAL. This handles multi-threaded
 * access by having separate lists for each thread.
 */
static inline void organism_completed_task(organism* o, int task) {
    o->energy += ENERGY_PER_TASK[task];
    o->tasks_completed[task]++;
    o->env->tasks_completed[get_thread_num()].counts[task]++;
}
...
static inline void inst_h_copy(organism* o) {
    ...
    unsigned int mutate = rand_r(&o->random_seed), inst = rand_r(&o->random_seed);
    uint8_t value = (mutate/(float)RAND_MAX <= o->env->mutation_prob) ?
        inst % NUM_INSTRUCTIONS : orig;
...
static inline void inst_h_divide(organism* o) { // NOTE: this is the only CPU instruction that truly requires the environment or location...
    ...
    // add the offspring to the list of organisms to be spawned at the end of the iteration
    environment_add_to_be_spawned(o->env, o->memory+start, offspring_size, o->location_i * o->env->world_size + o->location_j);
...
void execute_instruction(organism* o) {
    ...
    //printf("%u %c\n", o->instructions_executed, instruction + 'a');
...
///// Organism spawning helpers /////

void environment_add_to_be_spawned(environment* env, uint8_t* genome, uint32_t size, uint32_t loc) {
    // grab the next spawned organism slot for this thread
    spawned_organism* spawned = spawned_org_array_get_next(&env->spawned[get_thread_num()]);
    if (!spawned) { return; }

    // copy the genome and set the location of this spawned organism
    memcpy(spawned->genome, genome, size);
    spawned->size = size;
    spawned->loc = loc;
}
...
void environment_process_spawned_organisms(environment* env) {
    ...
    if (total > 0) {
        ...
        for (int i = 1; i < num_threads; i++) {
            ...
            memcpy(&spawned_arrs[0].data[offset], arr->data, arr->size * sizeof(spawned_organism));
            offset += arr->size;
            arr->size = 0;
        }
        ...
        // find the new locations for the spawned organisms
        for (uint32_t i = 0; i < total; i++) {
            spawned_organism* spawned = &array->data[i];
            uint32_t loc = spawned->loc, i = loc / world_size, j = loc % world_size;
            organism* o = &grid[loc];

            // figure out where to place the offspring
            int32_t i_off, j_off;
            do {
                i_off = rand_r(&o->random_seed) % 3 - 1;
                j_off = rand_r(&o->random_seed) % 3 - 1;
            } while ((i_off == 0 && j_off == 0) ||
                    (i + i_off == -1) || (i + i_off >= world_size) ||
                    (j + j_off == -1) || (j + j_off >= world_size));

            // spawn the offspring in the environment (but it may be out of bounds for the current rank)
            spawned->loc = (i+i_off)*world_size + j+j_off;
        }
